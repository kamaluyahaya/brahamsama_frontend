...
export default function ClientDetailPage() {
  ...
  async function fetchClientMotorcycles() {
    try {
  ...
  const handleDeleteMotorcycle = async (mcId: number) => {
    ...
    try {
      const res = await fetch(`/api/clients/${clientId}/motorcycles/${mcId}`, {
        method: 'DELETE'
      });
      if (res.ok) {
  ...
  const handleDeleteClient = async () => {
    if (!client) return;
    if (!confirm(`Are you sure you want to delete client record "${client.name}"? This action cannot be undone.`)) return;
    try {
    ...
    <div className="space-y-6 max-w-6xl mx-auto pb-12">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl md:text-2xl font-black tracking-tight text-slate-900 dark:text-white">{client.name}</h1>
              <span className="bg-violet-100 dark:bg-violet-950/70 text-violet-700 dark:text-violet-300 text-xs font-bold px-2.5 py-1 rounded-lg border border-violet-200 dark:border-violet-800">
                File: {client.file_no || 'N/A'}
              </span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-950/40 p-4 rounded-xl border border-slate-200/80 dark:border-slate-800/60">
              <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1.5">
                ...
                <input
                  ...
                  className="bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-3.5 py-2.5 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-violet-500/50 text-xs"
          ...
          <div className="bg-white dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 rounded-2xl p-6 shadow-sm space-y-4">
            ...
            {motorcycles.length === 0 ? (
              <div className="text-center py-8 bg-slate-50 dark:bg-slate-950/30 rounded-xl border border-dashed border-slate-200 dark:border-slate-800">
                        ...
                        <td className="px-3.5 py-3 whitespace-nowrap text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            ...
                            <button
                              ...
                              className="p-1.5 bg-rose-100 hover:bg-rose-200 dark:bg-rose-950/60 dark:hover:bg-rose-900/80 text-rose-600 dark:text-rose-400 rounded-lg transition-all"
      ...
      {showAssignModal && (
        <ModalPortal>
          ...
          >
            <div
              className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 w-full max-w-xl max-h-[90vh] flex flex-col rounded-2xl shadow-2xl overflow-hidden my-auto"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-xl px-3 py-2 text-slate-900 dark:text-white focus:ring-2 focus:ring-violet-500/50"
                    />
                  </div>
